...
var ApiServer = /** @class */ (function () {
    ...
    ApiServer.prototype.start = function (port) {
        ...
        // Initialize passport
        this.restify.use(passport.initialize());
        passport_1.jwtStrategy(passport);
        this.restify.get('/testar', passport.authenticate('jwt', { session: false }), function (req, res) {
            res.send('Hello');
        });
        controllers_1.CONTROLLERS.forEach(function (controller) { return controller.initialize(_this); });
        this.restify.listen(port, function () { return console.log("Servidor est\u00E1 rodando na porta " + port); });
    };
    return ApiServer;
}());
